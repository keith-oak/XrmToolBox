...
SHELL_PROJ="${REPO_ROOT}/src.macos/XrmToolBox.MacOS"
...
if [ -d "${PUBLISH_DIR}/Plugins" ]; then
  ...
  if [ -d "${REPO_ROOT}/src.macos/XrmToolBox.MacOS/bin/Release/net10.0/Plugins" ]; then
    cp -R "${REPO_ROOT}/src.macos/XrmToolBox.MacOS/bin/Release/net10.0/Plugins" "${APP}/Contents/MacOS/"
  fi
fi
