...
Replace the default Avalonia FluentTheme in `src.macos/XrmToolBox.MacOS` with a custom theme that follows Apple's Human Interface Guidelines (HIG) so the shell feels native on macOS. The same theme must degrade gracefully on Windows and Linux (no broken visuals, no missing fonts, just "looks tasteful" rather than "looks Mac").
...
src.macos/XrmToolBox.MacOS/bin/Debug/net10.0/XrmToolBox
