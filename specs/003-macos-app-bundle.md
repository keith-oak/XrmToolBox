...
- [ ] `scripts/build-macos-app.sh` (executable) that:
  1. Runs `dotnet publish src/Shell -c Release -r osx-arm64 --self-contained true -p:PublishSingleFile=false`
...
- [ ] `src/Shell/XrmToolBox.MacOS.csproj` declares the bundle metadata so `dotnet publish` picks up the version + icon path:
