...
XrmToolBox is a Dataverse / Dynamics 365 plugin host. The macOS-native fork (`src.macos/`) replaces the Windows-only WinForms shell with a cross-platform Avalonia + .NET 10 stack so the same binary runs on macOS (primary), Windows, and Linux.
...
2. **Plugin contract is sacred.** The `IXrmToolBoxPlugin` SDK surface (`src.macos/XrmToolBox.Extensibility.Core`) is the long-lived contract. Nothing in the shell may break a plugin that compiled cleanly against a previous SDK minor version.
...
dotnet build src.macos/XrmToolBox.MacOS.slnx -nologo -warnaserror > /dev/null 2>&1
src.macos/XrmToolBox.MacOS/bin/Debug/net10.0/XrmToolBox --probe   # must list ≥1 plugin
dotnet format src.macos/XrmToolBox.MacOS.slnx --verify-no-changes
